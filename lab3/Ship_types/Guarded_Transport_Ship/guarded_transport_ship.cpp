#include "guarded_transport_ship.hpp"
#include <cmath>

namespace Prog3 {

	unsigned int GuardedTransportShip::Shoot(double p1_x, double p1_y, double p2_x, double p2_y) {
	...
	void GuardedTransportShip::TakeDamage(unsigned int damage) {
	...
	void GuardedTransportShip::Move(Point& start, const Point& end) {				//перемещение за 1 секунду
	...
	void GuardedTransportShip::CurrentSpeedRange(double from, double to) {
	...
	double GuardedTransportShip::MaxSpeed(unsigned int weight) {
